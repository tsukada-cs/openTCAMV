...
from opentcamv import cli, data, params, output, rotation, tracking, aggregate, screening, records
...
frames, tname, yname, xname = data.open_frames(args)
mask = data.build_mask(args, frames)
nt = frames[tname].size
tg = data.compute_tg(args, nt)

setup = params.TrackingSetup.from_args(args, frames, args.varname)
names = output.axis_names(args)
xxg, yyg = output.build_seed_positions(args)
polar_trig = output.polar_trig(args)
costh, sinth = polar_trig if polar_trig is not None else (None, None)

coords = output.build_coords(args, frames, tname, tg)
pickup_it_rel, pickup_it_rel_v = output.pickup_indices(args, coords)

#%% Build one empty output Dataset per Omega
ofls, encodings = {}, {}
for omega in args.revrot:
    ofl, encoding = output.build_empty_dataset(args, frames, tname, coords, tg, setup)
    ofls[omega] = ofl
...
diagnostics = tracking.diagnostics_arg(args)
...
for j, tid0 in enumerate(tg.tolist()):
    ...
    lo, hi = tracking.window_bounds(tid0, setup, nt)
    t_win = t[lo:hi + 1]
    ...
    for omega in args.revrot:
        z_win = rotation.rotate_window(z_base, t_win, t0_local, omega) if omega else z_base
        fwd, bwd = tracking.track_at(
            tracker, setup, z_win, t_win, mask_win, grid, t0_local, lo, xxg, yyg, diagnostics=diagnostics
        )
        step = aggregate.aggregate_step(args, setup, fwd, bwd, tid0, t, grid, pickup_it_rel, pickup_it_rel_v)
        ...
        if not args.polar:
            ...
            vr, vt = aggregate.to_polar(step.vx, step.vy, costh, sinth)
            rtraj, atraj = aggregate.traj_to_polar(step.xtraj, step.ytraj)
            ofl["vr"].data[j] = vr
            ofl["vt"].data[j] = vt
...
#%% Per-Omega postprocessing (revrot restore -> screening -> records -> write)
for omega, ofl in ofls.items():
    screening.restore_revrot(ofl, args, omega, names)
    ofl = screening.screen(ofl, args, names)

    ofl = records.record_initpos(ofl, args, frames, tname, yname, xname, tg)
    ofl = records.record_alongtraj_and_cth(ofl, args, frames, tname, yname, xname, xxg, yyg)

    if args.out_score_ary and args.out_psr:
        around_ratio = 0.15
        ofl["psr"] = records.peak_to_sidelobe_ratio(ofl, around_ratio)
        ofl["psr"].attrs.update({"long_name": "peak-to-sidelobe ratio", "around_ratio": around_ratio, "units": ""})

    ofl.attrs.update(output.build_attrs(args, omega, script_path=__file__))
    ofl.attrs.update(setup.to_attrs(z, omega, mask))

    ofn = args.ofn.replace("<omega>", f"{omega:.4f}") if "<omega>" in args.ofn else args.ofn
    output.write(ofl, encodings[omega], ofn, args.complevel)
