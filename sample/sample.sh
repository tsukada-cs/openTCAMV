...
# Default output is a single file holding all Omega candidates along an
# `omega` dimension.
python ../scripts/10_conduct_tracking.py 2017_Lan_aeqd_sample.nc \
    ...
    -o='2017_Lan_ns7_nt1_candidates.nc' --ygran=-45:45 --xgran=-45:45 --traj_int=1 --Vs=10 \
...
## For one file per Omega instead of the combined default, add
## --split_omega and give -o a `<omega>` placeholder.

# 2-1. Finalize tracking. --omega is not needed: read from the `omega`
# dimension in 2017_Lan_ns7_nt1_candidates.nc.
